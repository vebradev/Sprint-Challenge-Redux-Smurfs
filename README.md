...
Action is an object that contains type and payload. Action is dispatched to reducer and tells it how state should be changed.
Reducer is a function that updates state data.
Store is a huge object that holds the applications state data. It's called 'single source of truth' because all the state of application is moved outside of components.


- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state is globally accessible and is kept in store. Component state, on the other hand, is kept inside component and can only be accessed by child components.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Redux-thunk is middleware which enables asyn javascript inside our applications. It allows to write action creators that return a function.
